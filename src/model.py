...
class DerainCycleGAN(nn.Module):
  def __init__(self, opts):
    ...
    self.pba_opt = torch.optim.Adam(self.pba.parameters(), lr=lr, betas=(0.5, 0.999), weight_decay=0.0001)    
  ...
  def initialize(self):
    ...
    self.pba.apply(networks.gaussian_weights_init)
  ...
  def set_scheduler(self, opts, last_ep=0):
    ...
    self.pba_sch = networks.get_scheduler(self.pba_opt, opts, last_ep)
  ...
  def test_forward(self, image1, image2=None, a2b=None):
    if a2b:
        self.mask_a = self.pba(image1)
        self.frame1_a, self.frame2_a, self.fake_A_encoded = self.genA.forward(image1, self.mask_a) 
    else:
        self.mask_a = self.pba(image1)
        batch_size, row, col = self.mask_a.size(0), self.mask_a.size(2), self.mask_a.size(3)
        noise = (torch.rand(batch_size, 1, row, col) * 0.01).cuda()
        self.mask_a = self.mask_a + noise
        self.frame1_b, self.frame2_b, self.fake_A_encoded = self.genB.forward(image1, self.mask_a)  
    return self.fake_A_encoded
  
  def test_forward_revert(self, image1, image2=None, a2b=None):
    if a2b:
        self.mask_a = self.pba(image1)
        batch_size, row, col = self.mask_a.size(0), self.mask_a.size(2), self.mask_a.size(3)
        noise = (torch.rand(batch_size, 1, row, col) * 0.01).cuda()
  ...
  def forward(self, ep, opts):
    ...
    self.mask_a = self.pba(self.real_A_encoded)
    self.frame1_a, self.frame2_a, self.fake_A_encoded = self.genA.forward(self.real_A_encoded, self.mask_a)  
    self.mask_b = self.pba(self.real_B_encoded)
    self.frame1_b, self.frame2_b, self.fake_B_encoded = self.genB.forward(self.real_B_encoded, self.mask_b) 

    # get perceptual loss
    self.perc_real_A = self.vgg(self.real_A_encoded).detach() #输入图像
    self.perc_fake_A = self.vgg(self.fake_A_encoded).detach() #去水图像
  ...
  def backward_EG(self, opts):
    ...
    # reconstructive loss
    fake = self.mask_a + self.fake_A_encoded
  ...
  def resume(self, model_dir, train=True):
    ...
    self.pba.load_state_dict(checkpoint['atten'])
    ...
    if train:
      ...
      self.pba_opt.load_state_dict(checkpoint['atten_opt'])
  ...
  def assemble_outputs(self):
    ...
    images_mask_a = self.normalize_image(self.mask_a).detach()
    images_mask_b = self.normalize_image(self.mask_b).detach()
   

    row1 = torch.cat((images_a[0:1, ::], images_a1[0:1, ::], images_a3[0:1, ::]),3)
    row2 = torch.cat((images_b[0:1, ::], images_b1[0:1, ::], images_b3[0:1, ::]),3)    
    row3 = torch.cat((images_mask_a[0:1, ::], images_mask_a[0:1, ::], images_mask_a[0:1, ::]),3)   
    row4 = torch.cat((images_mask_b[0:1, ::], images_mask_b[0:1, ::], images_mask_b[0:1, ::]),3) 
     
    return torch.cat((row1,row2),2), torch.cat((row3,row4), 2)

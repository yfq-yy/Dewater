...
def main():
  ...
  # define logger file
  logger_path = os.path.join(opts.result_dir, opts.name, 'log.txt')
  logger = Logger(logger_path)

  # daita loader
  logger.write('\n--- load dataset ---')
  dataset = dataset_unpair(opts)
  train_loader = torch.utils.data.DataLoader(dataset, batch_size=opts.batch_size, shuffle=True, num_workers=opts.nThreads) #1,8
  ...
  ep0 += 1
  logger.write('start the training at epoch %d'%(ep0))
  ...
  for ep in range(ep0, opts.n_ep):
    ...
    for it, (images_a, images_b) in enumerate(train_loader):
      ...
      if not opts.no_display_img:  #false
        saver.write_display(total_it, model)
      message = 'total_it: %d (ep %d, it %d), lr %08f, disA %04f, disB %04f, ganA %04f, ganB %04f, recA %04f, recB %04f, percp %04f, recons_loss %04f, attA %04f, attB %04f, total %04f' %  (total_it, ep, it, model.genA_opt.param_groups[0]['lr'], \
                                                      model.disA_loss, model.disB_loss, \
                                                      model.gan_loss_a, model.gan_loss_b, \
                                                      model.l1_recon_A_loss, model.l1_recon_B_loss, \
                                                      model.perceptual_loss, model.cons_loss, model.atten_loss_a, model.atten_loss_b, model.G_loss)
      total_it += 1
    ...
    for i, (input_val, target_val) in enumerate(loader_val, 0):        
        ...
        message1 = "[epoch %d][%d/%d] psnr: %.4f, ssim: %.4f, psnr_sum: %.4f, ssim_sum: %.4f" % (ep+1, i+1, len(loader_val), psnr_val, ssim_val.item(), psnr_sum, ssim_sum)
    ...
    logger.write("[epoch %d][%d/%d] psnr_avg: %.4f, ssim_avg: %.4f, best_psnr: %.4f, best_ssim: %.4f" %
            (ep+1, i+1, len(loader_val), psnr_avg, ssim_avg, best_psnr, best_ssim))
    logger.write('\n')

    if (ssim_avg >= best_ssim) and (psnr_avg >= best_psnr):
        best_psnr = psnr_avg
        best_ssim = ssim_avg         
        logger.write('--- save the model @ ep %d ---' % (ep))
        logger.write('\n')
